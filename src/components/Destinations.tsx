...
import { MapPin, Star, Plane, CalendarIcon } from "lucide-react";
import codes from "../lib/airport-codes";
...
const Destinations = () => {
  ...
  const handleViewFlights = (destination: any) => {
    setSelectedDestination(destination);
    setDialogOpen(true);
  };

  const handleSearch = () => {
    if (!from || !selectedDestination || !date) return;
    // Find the airport code for the selected destination city
    const destAirport = codes.find(
      (a) => a.city.toLowerCase() === selectedDestination.city.toLowerCase()
    );
    const toCode = destAirport ? destAirport.code : selectedDestination.city;
    navigate(`/flights?from=${from}&to=${toCode}&departure=${date}&passengers=${passengers}`);
    setDialogOpen(false);
  };
  ...
  return (
    <section className="relative py-24 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 overflow-hidden" id="destinations">
      ...
      <div className="container mx-auto px-4 relative z-10">
            ...
            <div key={index} className="group relative bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/20">
              ...
              <div className="relative z-10">
                ...
                <div className="p-6">
                  ...
                  <button onClick={() => handleViewFlights(destination)} className="group/btn inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 font-semibold transition-all duration-300 hover:gap-3">
                    View Flights
        ...
        {/* Single Dialog for all destinations */}
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {selectedDestination ? `Search Flights to ${selectedDestination.city}, ${selectedDestination.country}` : 'Search Flights'}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={e => { e.preventDefault(); handleSearch(); }} className="space-y-4">
              ...
              <div>
                ...
                <input value={selectedDestination ? (codes.find(a => a.city.toLowerCase() === selectedDestination.city.toLowerCase())?.code || selectedDestination.city) : ''} disabled className="w-full px-3 py-2 rounded border bg-gray-100 text-gray-700" />
              </div>
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-full justify-start text-left font-normal h-auto min-h-[40px] py-2 bg-white border-gray-300 hover:bg-gray-50"
                    >
              ...
              <div>
                ...
                <input type="number" min="1" value={passengers} onChange={e => setPassengers(e.target.value)} className="w-full px-3 py-2 rounded border" required />
              </div>
              <DialogFooter>
